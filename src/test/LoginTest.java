...
class LoginTest {
    ...
    void verifyDataFields(ResponseData response){
        System.out.print("Data fields verification: ");
        LoggedIn actual = (LoggedIn) response.obj;
    ...
    void positiveCase(String email, String password) throws IOException {
        ResponseData response = ReqresAPI.login(email, password);
    ...
    void negativeCaseUndefined(String email, String password) throws IOException {
        ...
        verifyError("user not found", response);
    }
    ...
    @DisplayName("Test login for existing user")
    @MethodSource("predefinedUsers")
    void validUser(User user) throws IOException {
        positiveCase(user.data.email, "password");
    }

    @ParameterizedTest(name="email={0}, password={1}")
    @DisplayName("Test login for undefined user")
    @CsvSource({
            "0123456789,password",
            "_!?@#$%^&*+-=<>(){}[],password",
            "©±ÆÐßæΣℜ∀≅,password",
